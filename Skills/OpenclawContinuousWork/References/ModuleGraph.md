# Module Graph

Auto-generated dependency graph between markdown modules.

## Nodes
- `AcceptanceTemplate.md`
- `ContinuousExecutionDirective.md`
- `GeneralRules.md`
- `ModuleSystem.md`
- `ModuleTemplate.md`
- `OptimizationChecklist.md`
- `OptimizationDirective.md`
- `OptimizationRules.md`
- `QualityRubric.md`
- `ReportingTemplate.md`

## Edges
- `AcceptanceTemplate.md` -> (none)
- `ContinuousExecutionDirective.md` -> (none)
- `GeneralRules.md` -> (none)
- `ModuleSystem.md` -> (none)
- `ModuleTemplate.md` -> (none)
- `OptimizationChecklist.md` -> (none)
- `OptimizationDirective.md` -> (none)
- `OptimizationRules.md` -> (none)
- `QualityRubric.md` -> (none)
- `ReportingTemplate.md` -> (none)

## Notes
- Rebuild with: `python Scripts/BuildModuleGraph.py`
- Pair with `BuildReferenceMap.py` after adding new modules.
